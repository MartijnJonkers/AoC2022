...
for line in input[1:]:                                    # go over all lines (skipping the 'cd /' command)
  ...
  if l[0] == "$":                                           # command (skip 'ls', is is not needed)
    ...
    if l[1] not in cur_ref.keys():      
      cur_ref[l[1]] = {"_files":{}, "_size":0}                # add new subdir to current location, no files yet
...
folders = {}
def build_folderlist(structure, prefix = ""):              # add full path prefix to catch double folder names
